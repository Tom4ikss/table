...
export const Form = () => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-6  mx-auto space-y-6 flex flex-col">
      {fieldsConfig.map((field) => {
        return <Fragment key={field.name}>
            <label htmlFor={field.name} className="mb-2 w-xs font-semibold text-slate-700">{field.placeholder}</label>
            <input {...register(field.name)} id={field.name} className="border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-slate-800 w-xs focus:border-transparenttext-slate-800"/>
